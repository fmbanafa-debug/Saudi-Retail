...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Usage limits logic
    const saved = localStorage.getItem('banafa_v2_usage');
  ...
  const handleAiAnalysis = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setCooldown(45);
        localStorage.setItem('banafa_v2_usage', JSON.stringify({ count: newCount, date: new Date().toDateString() }));
      } else {
        setAiResponse(data.error || 'خطأ في معالجة الطلب.');
      }
    } catch (err) {
      setAiResponse('تأكد من إعداد GEMINI_API_KEY في Vercel.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-slate-50 text-slate-900 overflow-x-hidden">
      {/* Dynamic Navigation */}
        ...
        {activeTab === 'overview' && (
            <div className="space-y-12 animate-in fade-in duration-700">
                <div className="text-center mb-16">
                    <h2 className="text-4xl md:text-6xl font-black text-slate-900 mb-6 leading-tight">صراع القيمة<br/><span className="text-indigo-600">في التجزئة السعودية</span></h2>
                </div>
      ...
      {/* AI Modal with Protection */}
      {isAiModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-900/80 backdrop-blur-md" onClick={() => setIsAiModalOpen(false)}></div>
          <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl relative overflow-hidden flex flex-col max-h-[85vh] animate-in slide-in-from-bottom-8">
            ...
            <div className="p-8 overflow-y-auto flex-1 space-y-6">
              <div className="flex gap-2">
                ...
                <div className="px-4 py-1.5 rounded-full text-[10px] font-black bg-slate-100 text-slate-600 uppercase">
                  الطلبات المتبقية: {DAILY_LIMIT - dailyRequests}
                </div>
              </div>
            ...
            <div className="p-8 bg-white border-t border-slate-100">
              ...
              {dailyRequests >= DAILY_LIMIT && <p className="text-[10px] text-red-500 mt-2 font-bold text-center">وصلت للحد اليومي (15 طلباً). يرجى العودة غداً.</p>}
            </div>
          </div>
        </div>
      )}
